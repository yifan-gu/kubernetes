...
#   SALT_TAR_URL
#   SALT_TAR_HASH
#   TEMPLATES_TAR_URL (when distro == coreos)
#   TEMPLATES_TAR_HASH (when distro == coreos)
function upload-server-tars() {
  ...
  if [[ "${OS_DISTRIBUTION}" == "coreos" ]]; then
    echo "+++ Staging templates tar to Google Storage: ${staging_path}"
    local templates_tar="${KUBE_TEMP}/kubernetes-templates.tar.gz"
    tar czfC ${templates_tar} "${KUBE_ROOT}/cluster/gce/${OS_DISTRIBUTION}" "templates"
    TEMPLATES_TAR_HASH=$(sha1sum-file "${templates_tar}")
    local templates_gs_url="${staging_path}/${templates_tar##*/}"
    copy-if-not-staged "${staging_path}" "${templates_gs_url}" "${templates_tar}" "${TEMPLATES_TAR_HASH}"
    TEMPLATES_TAR_URL="${templates_gs_url/gs:\/\//https://storage.googleapis.com/}"
  fi
}
...
TEMPLATES_TAR_URL: $(yaml-quote ${TEMPLATES_TAR_URL})
EOF
  fi
}
