...
func (kl *Kubelet) probeContainer(pod *api.Pod, container api.Container, containerID string) (probe.Result, error) {
	// Probe liveness.
	...
	// Probe readiness.
	ready, err := kl.probeContainerReadiness(pod, container, containerID)
...
func (kl *Kubelet) probeContainerLiveness(pod *api.Pod, container api.Container, containerID string) (probe.Result, error) {
	p := container.LivenessProbe
	if p == nil {
		return probe.Success, nil
	}
	status, found := pod.Status.Info[containerID]
	if !found {
...
func (kl *Kubelet) probeContainerReadiness(pod *api.Pod, container api.Container, containerID string) (probe.Result, error) {
...
func (kl *Kubelet) runProbeWithRetries(p *api.Probe, pod *api.Pod, container api.Container, retires int) (probe.Result, error) {
	var err error
	var result probe.Result
	for i := 0; i < retires; i++ {
		result, err = kl.runProbe(p, pod, container)
...
func (kl *Kubelet) runProbe(p *api.Probe, pod *api.Pod, container api.Container) (probe.Result, error) {
	...
	if p.HTTPGet != nil {
		...
		host, port, path := extractGetParams(p.HTTPGet, pod.Status, port)
