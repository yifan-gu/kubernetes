...
func runCommand(subCommand string, args ...string) ([]string, error) {
	...
	output, err := exec.Command(rocketBinName, allArgs...).Output()
	if err != nil {
		return nil, err
	}
...
func getPodsState() (map[string]string, error) {
	result := make(map[string]string)
	output, err := runCommand("list", "--no-legend")
	if err != nil {
		return nil, err
	}
...
func getPodStatus(pod *api.Pod) error {
	// TODO(yifan) Cache this.
	podStates, err := getPodsState()
	if err != nil {
		return err
	}
...
func makePod(unitName string) (*api.Pod, error) {
...
func (r *RocketRuntime) preparePod(pod *api.BoundPod) (string, error) {
	...
	output, err := runCommand("prepare", images...)
	if err != nil {
		return "", err
	}
	if len(output) != 1 {
