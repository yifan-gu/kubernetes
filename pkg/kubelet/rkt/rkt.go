...
func (r *Runtime) preparePod(pod *api.Pod, pullSecrets []api.Secret) (string, *kubecontainer.Pod, error) {
	...
	units := []*unit.UnitOption{
		newUnitOption("Service", "Slice", "rkt.slice"),
...
func (r *Runtime) GarbageCollect(gcPolicy kubecontainer.ContainerGCPolicy) error {
	...
	for _, pod := range resp.Pods {
		if pod.State != rktapi.PodState_POD_STATE_RUNNING {
			uid := getPodUID(pod)
			if uid == types.UID("") {
				glog.Errorf("rkt: Cannot get the UID of pod %q, will remove it", pod.Id)
	...
	// Remove in parallel.
	var wg sync.WaitGroup
	errCh := make(chan error, len(removeCandidates))
	wg.Add(len(removeCandidates))
	for _, pod := range removeCandidates {
		go func(pod *rktapi.Pod) {
			defer wg.Done()
			if err := r.removePod(pod.Id); err != nil {
				errCh <- fmt.Errorf("rkt: Failed to clean up rkt pod %q: %v", pod.Id, err)
			}
		}(pod)
	}
	wg.Wait()
