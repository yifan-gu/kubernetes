...
func TestPreStopHooks(t *testing.T) {
	...
	for i, tt := range tests {
		...
		// Run pre-stop hooks.
		err := rkt.runPreStopHooks(tt.pod, tt.runtimePod)
		assert.Equal(t, tt.err, err, testCaseHint)
...
func TestPostStartHooks(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// Case 1, containers with post-start hook.
			&api.Pod{
				ObjectMeta: api.ObjectMeta{
					Name:      "pod-1",
					Namespace: "ns-1",
		...
		{
			// Case 2, one container with invalid post-start hook.
			&api.Pod{
				ObjectMeta: api.ObjectMeta{
					Name:      "pod-1",
					Namespace: "ns-1",
	...
	for i, tt := range tests {
		...
		err := rkt.runPostStartHooks(tt.pod, tt.runtimePod)
		assert.Equal(t, tt.err, err, testCaseHint)
